document.addEventListener('DOMContentLoaded', function () {
    ...
    // Atualiza o valor máximo do empréstimo quando o valor aproximado do imóvel é inserido
    document.getElementById('valor-aprox').addEventListener('input', function () {
      const valorImovel = valorAproxInput.getRawValue(); // Pega o valor sem máscara
      if (valorImovel) {
        // Remove qualquer caractere não numérico, incluindo prefixos
        const valorImovelFloat = parseFloat(valorImovel.replace(/\D/g, '')); 

        if (!isNaN(valorImovelFloat) && valorImovelFloat > 0) {  // Verifica se o valor é um número válido
